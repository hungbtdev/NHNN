...
app.directive('sgNumberInput', ['$filter', '$locale', function ($filter, $locale) {
    return {
        require: 'ngModel',
        restrict: "A",
...
app.directive('fnumber', ['$filter', function ($filter) {
    return {
        require: '?ngModel',
        link: function (scope, elem, attrs, ctrl) {
            ...
            ctrl.$formatters.unshift(function (a) {
                return $filter(attrs.fnumber)(ctrl.$modelValue)
            });
            ...
            elem.on('change', function (event) {
                ...
                elem.val($filter(attrs.fnumber)(plainNumber));
            });
        }
    };
}]);
